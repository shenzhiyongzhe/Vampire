    ...
    protected override IEnumerator StartAttack()
    {
        while (true)
        {
            for (int i = 0; i < WeaponNum; i++)
            {
                ...
                obj.SetActive(true);
                obj.GetComponent<Rigidbody2D>().AddForce(new Vector2(4000 * Mathf.Cos(Mathf.PI / WeaponNum * i + Mathf.PI / 4) * AttackSpeed, 
                    10000 * Mathf.Sin(Mathf.PI / WeaponNum * i + Mathf.PI / 4) * AttackSpeed), ForceMode2D.Force);
            }
